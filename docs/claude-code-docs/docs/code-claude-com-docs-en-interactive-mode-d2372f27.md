...
fetched_at: "2026-07-12T00:15:45.524Z"
---
##

[​

](https://code.claude.com/docs/en/interactive-mode#keyboard-shortcuts)

Keyboard shortcuts

Keyboard shortcuts may vary by platform and terminal. In [fullscreen rendering](https://code.claude.com/docs/en/fullscreen), press `?` in the transcript viewer to see available shortcuts there.**macOS users**: Option/Alt key shortcuts (`Alt+B`, `Alt+F`, `Alt+Y`, `Alt+M`, `Alt+P`) require configuring Option as Meta in your terminal:
...
| `Ctrl+T` | Toggle Claude’s task checklist | Show or hide [Claude’s to-do checklist](https://code.claude.com/docs/en/interactive-mode#task-list) in the status area. This is not the background-task view; use [`/tasks`](https://code.claude.com/docs/en/commands) to see running shells and subagents |
...
| `Shift+Tab` or `Alt+M` (some configurations) | Cycle permission modes | Cycle through `default` (labeled Manual in the mode indicator), `acceptEdits`, `plan`, and any modes you have enabled, such as `auto` or `bypassPermissions`. See [permission modes](https://code.claude.com/docs/en/permission-modes). |
...
](https://code.claude.com/docs/en/interactive-mode#theme-and-display)
...
| --- | --- | --- |
| `/` at start | Command or skill | See [commands](https://code.claude.com/docs/en/interactive-mode#commands) and [skills](https://code.claude.com/docs/en/skills) |
...
](https://code.claude.com/docs/en/interactive-mode#voice-input)
...
](https://code.claude.com/docs/en/interactive-mode#visual-mode)
...
](https://code.claude.com/docs/en/interactive-mode#command-history)
...
The search loads the 100 most recent unique prompts in the selected scope, with duplicates collapsed to the newest occurrence. Matching prompts display with the search term highlighted, so you can find and reuse previous inputs. Accepting a match or canceling the search takes effect immediately, even while Claude Code is still loading the history. Before v2.1.202, accepting or canceling during that load could report an internal error.
...
](https://code.claude.com/docs/en/interactive-mode#how-backgrounding-works)
...
-   Background tasks are automatically cleaned up when Claude Code exits. Backgrounding the session instead of exiting it hands them to the background session, where they keep running. See [background a running session](https://code.claude.com/docs/en/agent-view#from-inside-a-session)
-   Background tasks are automatically terminated if output exceeds 5GB, with a note in stderr explaining why
-   As of v2.1.193, on macOS and Linux, running background tasks are terminated when the operating system signals memory pressure, provided the session has been idle for at least 30 minutes with no turn or subagent running. Set [`CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP`](https://code.claude.com/docs/en/env-vars) to `1` to turn this off
...
Earlier side questions from the same session appear as a dimmed list above the current answer. They stay out of the conversation history but remain visible in the overlay until you clear them. Once the answer appears, the overlay accepts these keys.
...
](https://code.claude.com/docs/en/interactive-mode#pr-review-status)
...
](https://code.claude.com/docs/en/interactive-mode#see-also)
