...
fetched_at: "2026-07-12T00:15:45.092Z"
---
...
| `Server error mid-response. The response above may be incomplete.` | [Server errors](https://code.claude.com/docs/en/errors#the-response-above-may-be-incomplete) |
| `Connection closed mid-response` / `Response stalled mid-stream` | [Server errors](https://code.claude.com/docs/en/errors#the-response-above-may-be-incomplete) |
...
| `Auto mode could not evaluate this action and is blocking it for safety` | [Server errors](https://code.claude.com/docs/en/errors#auto-mode-cannot-determine-the-safety-of-an-action) |
| `Auto mode classifier transcript exceeded context window` | [Server errors](https://code.claude.com/docs/en/errors#auto-mode-cannot-determine-the-safety-of-an-action) |
| `Agent terminated early due to an API error` | [Server errors](https://code.claude.com/docs/en/errors#agent-terminated-early-due-to-an-api-error) |
...
| `Usage credits required for 1M context` | [Usage limits](https://code.claude.com/docs/en/errors#usage-credits-required-for-1m-context) |
...
| `Could not resolve authentication method` | [Authentication](https://code.claude.com/docs/en/errors#could-not-resolve-authentication-method) |
...
| `Your organization has disabled API key authentication` | [Authentication](https://code.claude.com/docs/en/errors#your-organization-has-disabled-api-key-authentication) |
| `Your organization has disabled Claude subscription access` | [Authentication](https://code.claude.com/docs/en/errors#your-organization-has-disabled-claude-subscription-access) |
| `Routines are disabled by your organization's policy` | [Authentication](https://code.claude.com/docs/en/errors#routines-are-disabled-by-your-organizations-policy) |
| `Remote Control is only available when using Claude via api.anthropic.com` | [Authentication](https://code.claude.com/docs/en/errors#remote-control-requires-the-anthropic-api) |
...
| `AWS credentials expired or invalid` | [Authentication](https://code.claude.com/docs/en/errors#aws-credentials-expired-or-invalid) |
| `AWS authentication failed` | [Authentication](https://code.claude.com/docs/en/errors#aws-authentication-failed) |
| `Unable to connect to API` | [Network](https://code.claude.com/docs/en/errors#unable-to-connect-to-api) |
| `Waiting for API response · will retry in` | [Automatic retries](https://code.claude.com/docs/en/errors#automatic-retries), or [Network](https://code.claude.com/docs/en/errors#unable-to-connect-to-api) if it persists |
| `SSL certificate verification failed` | [Network](https://code.claude.com/docs/en/errors#ssl-certificate-errors) |
| `SSL certificate error (...)` during login or startup | [Network](https://code.claude.com/docs/en/errors#ssl-certificate-errors) |
| `403` with `x-deny-reason: host_not_allowed` in a cloud or routine session | [Network](https://code.claude.com/docs/en/errors#host-not-allowed-in-a-cloud-session) |
| `Couldn't reconnect to your Remote Control session` | [Network](https://code.claude.com/docs/en/errors#couldnt-reconnect-to-your-remote-control-session) |
...
| `Unable to resize image` | [Request errors](https://code.claude.com/docs/en/errors#unable-to-resize-image) |
...
| `Model ... is not a recognized model id` | [Request errors](https://code.claude.com/docs/en/errors#model-is-not-a-recognized-model-id) |
...
| `Model ... is restricted by your organization's settings` | [Request errors](https://code.claude.com/docs/en/errors#model-is-restricted-by-your-organizations-settings) |
...
| `Claude Code is unable to respond to this request, which appears to violate our Usage Policy` | [Request errors](https://code.claude.com/docs/en/errors#usage-policy-refusal) |
| `<model> has safety measures that flagged this message for a cybersecurity topic` | [Request errors](https://code.claude.com/docs/en/errors#safety-measures-flagged-a-cybersecurity-topic) |
| `Installation was killed before it could finish (exit code 137)` | [Installation errors](https://code.claude.com/docs/en/errors#installation-was-killed-before-it-could-finish) |
| `The connection dropped while downloading the update` | [Installation errors](https://code.claude.com/docs/en/errors#the-connection-dropped-while-downloading-the-update) |
| `Download timed out: exceeded the total deadline` | [Installation errors](https://code.claude.com/docs/en/errors#the-connection-dropped-while-downloading-the-update) |
| `--bg and --print conflict` | [Command-line errors](https://code.claude.com/docs/en/errors#command-line-errors) |
| `Error: --json-schema is not a valid JSON Schema` | [Command-line errors](https://code.claude.com/docs/en/errors#command-line-errors) |
| `Could not import <server>: <reason>` | [Command-line errors](https://code.claude.com/docs/en/errors#could-not-import-a-server-from-claude-desktop) |
| `Marketplace "<name>" is registered from an untrusted source` | [Plugin errors](https://code.claude.com/docs/en/errors#marketplace-is-registered-from-an-untrusted-source) |
| `Ignoring N permissions.allow entries from ... this workspace has not been trusted` | [Configuration warnings](https://code.claude.com/docs/en/errors#workspace-has-not-been-trusted) |
...
-   If the error persists with no posted incident, run `/feedback` so Anthropic can investigate with your request details. See [Report an error](https://code.claude.com/docs/en/errors#report-an-error) if `/feedback` is unavailable in your environment.
...
-   If the same error appears before any visible output, Claude Code retries the request instead of finalizing it. See [Automatic retries](https://code.claude.com/docs/en/errors#automatic-retries).
...
-   This is transient and unrelated to [auto mode eligibility](https://code.claude.com/docs/en/permission-modes#eliminate-prompts-with-auto-mode); you don’t need to change settings
...
In an interactive session, auto mode falls back to a normal permission prompt for that action so you can approve or deny it manually. In [non-interactive mode](https://code.claude.com/docs/en/headless) the run aborts because the transcript only grows and retrying can’t succeed. **What to do:**
...
-   Match the error detail after the colon to its own section on this page, such as [Usage limits](https://code.claude.com/docs/en/errors#usage-limits) or [Server errors](https://code.claude.com/docs/en/errors#server-errors), and follow that section’s steps
...
-   If the error persists after `/model`, a 1M model ID may be set elsewhere. See [There’s an issue with the selected model](https://code.claude.com/docs/en/errors#theres-an-issue-with-the-selected-model) for the configuration locations to check in priority order.
...
-   For the Agent SDK, see [authentication setup](https://code.claude.com/docs/en/agent-sdk/overview#get-started)
...
Environment variables and `apiKeyHelper` take precedence over `/login`, so running `/login` alone doesn’t help while either is still supplying a key. See [Authentication precedence](https://code.claude.com/docs/en/authentication#authentication-precedence). **What to do:**
...
As of v2.1.199, a certificate validation failure isn’t retried, so this error appears on the first attempt instead of after the full [retry budget](https://code.claude.com/docs/en/errors#automatic-retries). Earlier versions spent a few minutes retrying before showing it. Transient TLS conditions, such as a handshake timeout, still retry. During `/login` and the startup connectivity check, the same failure is reported with the OpenSSL code and the fix inline:
...
You may also see a TLS certificate that doesn’t match the destination’s real certificate. The cloud environment routes outbound traffic through a proxy that enforces the network policy, so a mismatched certificate means the proxy terminated the connection, not the destination. This is not a client-side network problem. Cloud sessions and [routines](https://code.claude.com/docs/en/routines) run inside a sandboxed environment whose outbound traffic is filtered to the environment’s allowlist. The **Default** environment uses **Trusted** access, which permits the [default allowlist](https://code.claude.com/docs/en/claude-code-on-the-web#default-allowed-domains) of package registries, cloud provider APIs, container registries, and common development domains but blocks everything else. **What to do:**
...
-   In the **Update cloud environment** dialog, change **Network access** from **Trusted** to **Custom**, then add the blocked domain to **Allowed domains**. Enter one domain per line. Check **Also include default list of common package managers** to keep the [default allowlist](https://code.claude.com/docs/en/claude-code-on-the-web#default-allowed-domains) alongside your custom domains. Select **Full** instead if you want unrestricted access.
-   Click **Save changes**. The next run uses the updated allowlist.

See [Network access](https://code.claude.com/docs/en/claude-code-on-the-web#network-access) for access levels and the default allowlist. Local CLI sessions are not affected by this policy.
...
-   A model saved before v2.1.200 isn’t repaired by this check. If a stale value keeps coming back, remove it from the locations listed under [There’s an issue with the selected model](https://code.claude.com/docs/en/errors#theres-an-issue-with-the-selected-model).
...
-   If you hit this in the [Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview), upgrade the SDK package instead. Opus 4.8 needs TypeScript SDK v0.3.154 or later and Python SDK v0.2.88 or later. Sonnet 5 needs TypeScript SDK v0.3.197 or later
...
-   On [Amazon Bedrock](https://code.claude.com/docs/en/amazon-bedrock), [Google Cloud’s Agent Platform](https://code.claude.com/docs/en/google-vertex-ai), and [Microsoft Foundry](https://code.claude.com/docs/en/microsoft-foundry), a cybersecurity flag produces the [Usage Policy refusal](https://code.claude.com/docs/en/errors#usage-policy-refusal) message instead.
-   [Non-interactive mode](https://code.claude.com/docs/en/headless) omits the `/feedback` sentence.
...
-   [Automatic model fallback](https://code.claude.com/docs/en/model-config#automatic-model-fallback) on Fable 5 moves the session to the default Opus model and shows a notice in the transcript

The Model selection check below catches the second and third cases; the first appears as a transcript notice rather than a `/model` change. [Model configuration](https://code.claude.com/docs/en/model-config) explains when each fallback applies. Check these first:
