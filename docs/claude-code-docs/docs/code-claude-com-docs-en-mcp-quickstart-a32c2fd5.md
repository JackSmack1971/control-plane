...
fetched_at: "2026-07-12T00:15:37.293Z"
---
...
You can also add MCP servers from other surfaces, including the desktop app, VS Code, and the web. See [Connect from other surfaces](https://code.claude.com/docs/en/mcp-quickstart#connect-from-other-surfaces).
...
](https://code.claude.com/docs/en/mcp-quickstart#before-you-begin)
...
| `! Needs authentication` | The server is reachable but needs a browser sign-in, or a token passed with `--header`. See [Connect a server that requires sign-in](https://code.claude.com/docs/en/mcp-quickstart#connect-a-server-that-requires-sign-in) |
| `✗ Failed to connect` | Server didn’t respond. See [Troubleshooting](https://code.claude.com/docs/en/mcp-quickstart#troubleshooting) |
| `✗ Connection error` | The connection attempt threw an error. See [Troubleshooting](https://code.claude.com/docs/en/mcp-quickstart#troubleshooting) |
| `⏸ Pending approval` | A project-scoped server you haven’t approved yet. See [Edit .mcp.json directly](https://code.claude.com/docs/en/mcp-quickstart#edit-mcp-json-directly) |
...
](https://code.claude.com/docs/en/mcp-quickstart#where-servers-are-saved)
...
-   [Add a local server](https://code.claude.com/docs/en/mcp-quickstart#add-a-local-server): run a program on your machine instead of connecting to a URL.
-   [Edit `.mcp.json` directly](https://code.claude.com/docs/en/mcp-quickstart#edit-mcp-json-directly): write the JSON entry yourself instead of using the command.
-   [Connect a server that requires sign-in](https://code.claude.com/docs/en/mcp-quickstart#connect-a-server-that-requires-sign-in): add a hosted server that needs a browser sign-in before its tools work.
...
On Windows, `~/.claude.json` resolves to `%USERPROFILE%\.claude.json`, typically `C:\Users\YourName\.claude.json`. If you’ve set [`CLAUDE_CONFIG_DIR`](https://code.claude.com/docs/en/env-vars), Claude Code reads `.claude.json` from inside that directory instead. Run `claude mcp get claude-code-docs` to see which scope holds a server’s definition. For how the scopes interact when the same server is defined in more than one, see [MCP installation scopes](https://code.claude.com/docs/en/mcp#mcp-installation-scopes).

##

[​

](https://code.claude.com/docs/en/mcp-quickstart#change-server-scope)
...
](https://code.claude.com/docs/en/mcp-quickstart#use-a-server-in-all-your-projects)
...
](https://code.claude.com/docs/en/mcp-quickstart#share-a-server-with-your-team)
...
](https://code.claude.com/docs/en/mcp-quickstart#additional-mcp-server-examples)
...
](https://code.claude.com/docs/en/mcp-quickstart#add-a-local-server)

Add a local server

A local stdio server is a program Claude Code starts as a subprocess on your machine, rather than a service it reaches over a URL. Use one for tools that need access to local resources like a browser, your filesystem, or a database socket. The [Playwright MCP server](https://github.com/microsoft/playwright-mcp) is a good one to try: it gives Claude a browser it can navigate, click, and read, and it needs no account. It runs through `npx`, so it requires [Node.js](https://nodejs.org/en/download) 18 or later.
...
Select `sentry` from the list, press Enter, and choose `Authenticate`. Your browser opens to Sentry’s sign-in page. Approve the connection there.Back in Claude Code, the server’s status changes to connected. If sign-in fails or the browser doesn’t open, see [Troubleshooting](https://code.claude.com/docs/en/mcp-quickstart#troubleshooting).
...
Every file in the [scope table](https://code.claude.com/docs/en/mcp-quickstart#find-your-configuration-on-disk) uses the same JSON format for server entries. This section edits `.mcp.json`, the project-scope file. It’s the one most worth writing by hand because it’s checked into the repository, where it doubles as configuration-as-code for your team. Create `.mcp.json` in your project root. The example below defines both servers from this guide, the hosted docs server reached over HTTP and the Playwright server as a local `stdio` process:
...
After saving the file, start a new Claude Code session in the project. Claude Code reads `.mcp.json` at startup. The first time Claude Code sees a project-scoped server, it asks you to approve it. The prompt exists so a repository you clone can’t launch processes on your machine without your consent. Approve the prompt, or run `/mcp` to approve later if you missed it. Once you’ve approved, run `/mcp` and check that the servers show as connected. If one shows an error instead, see [Troubleshooting](https://code.claude.com/docs/en/mcp-quickstart#troubleshooting).

##

[​

](https://code.claude.com/docs/en/mcp-quickstart#connect-from-other-surfaces)
...
-   **Claude Code on the web**: reads `.mcp.json` from your repository. See [Edit .mcp.json directly](https://code.claude.com/docs/en/mcp-quickstart#edit-mcp-json-directly).
...
-   A `401` or `403`: the server is up and you need to authenticate. Use the browser sign-in in [Connect a server that requires sign-in](https://code.claude.com/docs/en/mcp-quickstart#connect-a-server-that-requires-sign-in), or for servers that take a token instead, like GitHub’s, pass it with `--header "Authorization: Bearer <token>"` on the `claude mcp add` command.
...
](https://code.claude.com/docs/en/mcp-quickstart#next-steps)
...
[Isolate sessions with worktrees](https://code.claude.com/docs/en/worktrees)[Reference](https://code.claude.com/docs/en/mcp)
