...
fetched_at: "2026-07-12T00:15:32.411Z"
---
...
**When:** Session start.**What loads:** Tool names from connected servers. Full JSON schemas stay deferred until Claude needs a specific tool.**Context cost:** [Tool search](https://code.claude.com/docs/en/mcp#scale-with-mcp-tool-search) is on by default, so idle MCP tools consume minimal context.

Run `/mcp` to see connection status and token costs per server. Claude Code [reconnects to remote servers automatically](https://code.claude.com/docs/en/mcp#automatic-reconnection) if they drop, and you can disconnect servers you’re not actively using.
...
The LSP tool is inactive until you install a [code intelligence plugin](https://code.claude.com/docs/en/discover-plugins#code-intelligence) for your language.
