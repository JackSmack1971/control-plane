...
fetched_at: "2026-07-12T00:15:38.988Z"
---
...
A **Default** environment is provided with **Trusted** network access, which allows the [default set](https://code.claude.com/docs/en/claude-code-on-the-web#default-allowed-domains) of package registries, cloud provider APIs, container registries, and common development domains, but blocks everything else. If your routine needs to reach your own services or a domain outside that list, edit the environment’s [network access](https://code.claude.com/docs/en/claude-code-on-the-web#network-access) before running. To use a separate environment, [create one](https://code.claude.com/docs/en/claude-code-on-the-web#configure-your-environment) first.
...
Each routine runs in a [cloud environment](https://code.claude.com/docs/en/claude-code-on-the-web#the-cloud-environment) that controls network access, environment variables, and setup scripts. The routine inherits the environment’s network policy on every run. The **Default** environment uses **Trusted** network access: the [default allowlist](https://code.claude.com/docs/en/claude-code-on-the-web#default-allowed-domains) of package registries, cloud provider APIs, container registries, and common development domains is reachable, but arbitrary domains are not. Outbound requests to other hosts fail with `403` and `x-deny-reason: host_not_allowed`. MCP connector traffic is routed through Anthropic’s servers, so the connectors you add to the routine work without adding their hosts to **Allowed domains**. Remove any connectors you don’t need under [Connectors](https://code.claude.com/docs/en/routines#connectors). To allow additional domains:
...
See [Network access](https://code.claude.com/docs/en/claude-code-on-the-web#network-access) for details on access levels and the default allowlist.
...
](https://code.claude.com/docs/en/routines#troubleshooting)
...
](https://code.claude.com/docs/en/routines#/schedule-shows-%E2%80%9Cno-commands-match%E2%80%9D-or-%E2%80%9Cunknown-command%E2%80%9D)
...
](https://code.claude.com/docs/en/routines#%E2%80%9Droutines-are-disabled-by-your-organization%E2%80%99s-policy%E2%80%9D)
