...
-   **Model**: each model has its own cache. Switching models recomputes the entire request even when the content is identical. See [Switching models](https://code.claude.com/docs/en/prompt-caching#switching-models) below.
-   **Effort level**: each effort level has its own cache for the same model. Changing it mid-session recomputes the entire request, and Claude Code asks you to confirm before applying the change. See [Changing effort level](https://code.claude.com/docs/en/prompt-caching#changing-effort-level) below.
...
For what each provider stores and processes, see [data usage](https://code.claude.com/docs/en/data-usage). Wherever the cache lives, entries expire after a period of inactivity, and [Cache lifetime](https://code.claude.com/docs/en/prompt-caching#cache-lifetime) below covers the TTL and how to extend it.
...
-   [Turning on fast mode](https://code.claude.com/docs/en/prompt-caching#turning-on-fast-mode)
-   [Connecting or disconnecting an MCP server](https://code.claude.com/docs/en/prompt-caching#connecting-or-disconnecting-an-mcp-server)
-   [Enabling or disabling a plugin](https://code.claude.com/docs/en/prompt-caching#enabling-or-disabling-a-plugin)
-   [Denying an entire tool](https://code.claude.com/docs/en/prompt-caching#denying-an-entire-tool)
-   [Compacting the conversation](https://code.claude.com/docs/en/prompt-caching#compacting-the-conversation)
-   [Upgrading Claude Code](https://code.claude.com/docs/en/prompt-caching#upgrading-claude-code)

###

[​

](https://code.claude.com/docs/en/prompt-caching#switching-models)

Switching models

Each model has its own cache. Switching with [`/model`](https://code.claude.com/docs/en/model-config#setting-your-model) means the next request reads the entire conversation history with no cache hits, even though the content is identical. The [`opusplan` model setting](https://code.claude.com/docs/en/model-config#opusplan-model-setting) resolves to Opus during plan mode and Sonnet during execution, so each plan-mode toggle is a model switch and starts a fresh cache. [Automatic model fallback](https://code.claude.com/docs/en/model-config#automatic-model-fallback) on Fable 5 is also a model switch. When a safety classifier flags a request, Claude Code re-runs it on the default Opus model and the session continues there.

###

[​

](https://code.claude.com/docs/en/prompt-caching#changing-effort-level)
...
-   [Editing files in your repository](https://code.claude.com/docs/en/prompt-caching#editing-files-in-your-repository)
-   [Editing CLAUDE.md mid-session](https://code.claude.com/docs/en/prompt-caching#editing-claude-md-mid-session)
-   [Changing output style](https://code.claude.com/docs/en/prompt-caching#changing-output-style)
-   [Changing permission mode](https://code.claude.com/docs/en/prompt-caching#changing-permission-mode)
-   [Invoking skills and commands](https://code.claude.com/docs/en/prompt-caching#invoking-skills-and-commands)
-   [Running `/recap`](https://code.claude.com/docs/en/prompt-caching#running-%2Frecap)
-   [Rewinding the conversation](https://code.claude.com/docs/en/prompt-caching#rewinding-the-conversation)
-   [Spawning a subagent](https://code.claude.com/docs/en/prompt-caching#subagents-and-the-cache)

###

[​

](https://code.claude.com/docs/en/prompt-caching#editing-files-in-your-repository)
...
](https://code.claude.com/docs/en/prompt-caching#changing-output-style)
...
](https://code.claude.com/docs/en/prompt-caching#changing-permission-mode)

Changing permission mode

Switching between [permission modes](https://code.claude.com/docs/en/permission-modes), such as from default to accept edits, does not change the system prompt or tool definitions, so mode changes are cache-safe. The exception is plan mode with the [`opusplan`](https://code.claude.com/docs/en/model-config#opusplan-model-setting) model setting, which switches the model between Opus and Sonnet as you enter or leave plan mode. That makes the mode toggle a [model switch](https://code.claude.com/docs/en/prompt-caching#switching-models).

###

[​

](https://code.claude.com/docs/en/prompt-caching#invoking-skills-and-commands)
...
](https://code.claude.com/docs/en/prompt-caching#running-/recap)
...
](https://code.claude.com/docs/en/prompt-caching#rewinding-the-conversation)

Rewinding the conversation

[`/rewind`](https://code.claude.com/docs/en/checkpointing) truncates your conversation back to an earlier turn. The remaining history is the same content the cache was built from at that point, and the system prompt and project context layers are unchanged, so the next request hits the earlier cache entry. Every turn since then has read through that prefix, which kept the entry warm even if the original turn was longer ago than the TTL. Restoring file checkpoints alongside the conversation has no separate effect on the cache. File contents enter context only when Claude reads them, the same as [editing files in your repository](https://code.claude.com/docs/en/prompt-caching#editing-files-in-your-repository).
...
](https://code.claude.com/docs/en/prompt-caching#on-an-api-key-or-third-party-provider)
...
](https://code.claude.com/docs/en/prompt-caching#override-the-ttl)
...
](https://code.claude.com/docs/en/prompt-caching#check-cache-performance)
...
A high read-to-creation ratio means caching is working well. If creation stays high turn after turn, something is changing in your prefix. The [actions that invalidate the cache](https://code.claude.com/docs/en/prompt-caching#actions-that-invalidate-the-cache) section lists the usual causes. For visibility across an organization, the OpenTelemetry exporter reports cache read and creation tokens per user and session. See [Monitor usage](https://code.claude.com/docs/en/monitoring-usage) for the metric and event attribute reference.
...
A [subagent](https://code.claude.com/docs/en/sub-agents) starts its own conversation with its own system prompt and tool set, separate from the parent’s. It builds its own cache, starting with no cache hits on its first call and warming up across its own turns. Subagents use the five-minute TTL even on a subscription, since the automatic one-hour TTL applies to the main conversation. The parent’s cache is unaffected. From the parent’s side, the subagent’s call and result append to the conversation, leaving the parent’s prefix intact. A [fork](https://code.claude.com/docs/en/sub-agents#fork-the-current-conversation), by contrast, inherits the parent’s system prompt, tools, and conversation history exactly, so its first request reads the parent’s cache. The compaction summarization call described in [Compacting the conversation](https://code.claude.com/docs/en/prompt-caching#compacting-the-conversation) uses the same prefix-sharing approach.

##

[​

](https://code.claude.com/docs/en/prompt-caching#disable-prompt-caching)
...
To set caching policy across an organization, put any of these or the [TTL variables](https://code.claude.com/docs/en/prompt-caching#cache-lifetime) in the `env` block of [managed settings](https://code.claude.com/docs/en/settings#settings-files). For normal use, leave caching enabled.

##

[​

](https://code.claude.com/docs/en/prompt-caching#related-resources)
...
[Explore the context window](https://code.claude.com/docs/en/context-window)[Store instructions and memories](https://code.claude.com/docs/en/memory)
