...
def policy(tmp_path):
    ...
    (generated / "protected-paths.json").write_text(
        json.dumps(
            {
                "forbidden_roots": [".git"],
                "generated_file_roots": [".claude/control-plane/generated"],
            }
        )
    )
    (generated / "ownership-map.json").write_text(
        json.dumps({"ownership": {".claude/hooks/**": "hook-config"}})
    )
...
def test_guard_allows_declared_and_denies_protected_unowned_and_recovery(monkeypatch, tmp_path):
    ...
    policy(tmp_path)
    state = tmp_path / ".claude/control-plane/state"
    state.mkdir(parents=True)
    monkeypatch.setattr(guard, "ROOT", tmp_path)
    monkeypatch.setattr(guard, "STATE", state)
    monkeypatch.setattr(state_store, "ROOT", ROOT)
    monkeypatch.setattr(state_store, "STATE", state)
    state_store.write(
        "recovery.json",
        state_store.make("governed-change", "run-001", "active", "planned", "baseline", []),
    )
    state_store.write(
        "active-transaction.json",
        state_store.make(
            "governed-change", "run-001", "active", "planned", "baseline", [".claude/hooks/ok.py"]
        ),
    )
    assert guard.allowed(".claude/hooks/ok.py") == (True, "")
    assert guard.allowed(".claude/hooks/no.py") == (False, "CP506: undeclared write path")
    assert guard.allowed(".git/config") == (False, "CP501: forbidden write path")
    state_store.write(
        "recovery.json",
        state_store.make("governed-change", "run-001", "terminal", "rejected", "invalid", []),
    )
    assert guard.allowed(".claude/hooks/ok.py") == (False, "CP504: recovery state is incomplete")
...
def test_shell_read_is_ignored_and_write_without_target_is_denied():
    ...
    assert guard.write_paths(
        {"tool_name": "Bash", "tool_input": {"command": "echo x > /tmp/x"}}
    ) == [None]
    assert guard.write_paths(
        {"tool_name": "Bash", "tool_input": {"command": "git config x y"}}
    ) == [None]
...
def test_guard_denies_unowned_and_symlink_escape(monkeypatch, tmp_path):
    ...
    assert guard.allowed("README.md") == (False, "CP503: unowned write path")
    ...
    monkeypatch.setattr(
        ...
        io.StringIO(
            json.dumps(
                {
                    "tool_name": "Write",
                    "tool_input": {"file_path": str(tmp_path / ".claude/hooks/no.py")},
                }
            )
        ),
    )
...
def test_mutation_and_failure_hooks_persist_evidence(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda: {
            "tool_name": "Write",
            "tool_input": {"file_path": str(ROOT / ".claude/hooks/x.py")},
        },
    )
    ...
    record_failure.main()
    assert json.loads((tmp_path / "mutations.jsonl").read_text())["recovery"]["evidence_paths"] == [
        ".claude/hooks/x.py"
    ]
    assert (
        json.loads((tmp_path / "failures.jsonl").read_text())["recovery"]["details"]["error"]
        == "failed"
    )
...
def test_dirty_state_and_compaction_fail_closed(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        validate_dirty_state, "allowed", lambda _: (False, "CP506: undeclared write path")
    )
    monkeypatch.setattr(
        validate_dirty_state,
        "read",
        lambda _: {"lifecycle": "active", "recovery": {"evidence_paths": [".claude/hooks/x.py"]}},
    )
    monkeypatch.setattr(
        validate_dirty_state.subprocess,
        "run",
        lambda *_, **__: SimpleNamespace(stdout="?? .claude/hooks/x.py\n"),
    )
...
def test_hooks_do_not_invoke_claude_tools_recursively():
    for name in (
        ...
        "record_mutation.py",
        "record_failure.py",
        "validate_dirty_state.py",
    ):
        assert (
            "subprocess.run" not in (ROOT / ".claude/hooks" / name).read_text()
            or name == "validate_dirty_state.py"
        )
