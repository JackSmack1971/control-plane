...
def test_write_set_rejects_governed_change_not_declared(monkeypatch):
    ...
    assert errors == [
        "changed path is outside declared write set: .claude/control-plane/scripts/validate.py",
        "declared path was not changed: .claude/control-plane/scripts/check_write_set.py",
    ]
...
def test_write_set_enforces_changed_file_budget(monkeypatch):
    ...
    assert "declared write set exceeds changed-file budget" in errors
